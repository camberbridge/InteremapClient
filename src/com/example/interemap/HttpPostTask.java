...
public class HttpPostTask extends AsyncTask<Void, Void, Void> {
  ...
  private List<NameValuePair> post_params = null; //List：動的配列
  ...
  public HttpPostTask( Activity parent_activity, String post_url, Handler ui_handler )
  {
    ...
    post_params = new ArrayList<NameValuePair>(); //動的配列セット
  }
  ...
  // 追加
  public void addPostParam( String para_name, String para_value )
  {
    post_params.add(new BasicNameValuePair( para_name, para_value )); //配列の末尾にパラメータのキー&バリューのセットを格納する
  }
  ...
  protected Void doInBackground(Void... unused) {
    ...
    try {
      // 送信パラメータのエンコードを指定
      request.setEntity(new UrlEncodedFormEntity(post_params, request_encoding)); //サーバにパラメータを渡す
    ...
    try {
      httpClient.execute(request, response_handler);  //////////////////実際のPOSTリクエスト処理
